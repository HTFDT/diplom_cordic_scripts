...
static inline FileBits load_file_bits(const std::string& filename) {
...
static inline std::optional<uint64_t> find_first_bit(const FileBits& fb) {
...
static inline int64_t extract_word_tc(const FileBits& fb, uint64_t offset_bits, int bits) {
...
// ------------------------------------------------------------
// Общая структура эталонной трассы
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// Интерфейс провайдера эталона
// ------------------------------------------------------------

struct IReferenceProvider {
    virtual ~IReferenceProvider() = default;

    virtual ReferenceTrace compute_reference(
        int64_t input_fixed,
        int bits,
        int iterations
    ) const = 0;
...
// ------------------------------------------------------------
// Общие аргументы и состояние
// ------------------------------------------------------------

struct GenericRefComparerArgs : CommonArgs {
    int iterations = 32;
    ...
    std::string component_str = "result";
};

struct GenericRefComparerState : ReplState {
...
struct GenericRefComparerShell : AppShell<GenericRefComparerArgs, GenericRefComparerState> {
    static_assert(std::is_base_of<IReferenceProvider, TProvider>::value,
              "TProvider должен наследоваться от IReferenceProvider");

    TProvider provider;

    GenericRefComparerShell()
        : AppShell<GenericRefComparerArgs, GenericRefComparerState>(
            TProvider::app_name(),
            TProvider::app_description()
        )
    {
        ...
        app_.add_option("--component", args_.component_str,
...
protected:
    ...
    void load_args(int argc, char* argv[]) {
        AppShell<GenericRefComparerArgs, GenericRefComparerState>::load_args(argc, argv);
        args_.component = parse_component(args_.component_str);
    }

    void load_repl_input(const std::string& line) {
        args_.value_file = AppShell<GenericRefComparerArgs, GenericRefComparerState>::trim(line);
    }

    void validate_args() {
        AppShell<GenericRefComparerArgs, GenericRefComparerState>::validate_args();
    ...
    std::string compute() {
        ...
        ReferenceTrace ref = provider.compute_reference(
            input_fixed,
            args_.bits,
            args_.iterations
        );
        ...
        if (args_.component == CompareComponent::RESULT) {
            ref_value = ref.result;
        } else {
            if (!args_.iter_num.has_value()) {
                throw std::invalid_argument("Для component = x/y/z необходимо указать iter-num");
            }
            ...
            switch (args_.component) {
                case CompareComponent::X: vec = &ref.x; break;
                case CompareComponent::Y: vec = &ref.y; break;
                case CompareComponent::Z: vec = &ref.z; break;
                default: break;
            }

            if (vec == nullptr) {
                throw std::runtime_error("Внутренняя ошибка выбора компоненты");
            }

            if (it < 0 || it >= (int)vec->size()) {
                throw std::invalid_argument("iter-num выходит за диапазон эталонных итераций");
            }

            ref_value = (*vec)[(size_t)it];
        }
        ...
        bool ok = (truncate(file_value, args_.bits) ==
                   truncate(ref_value, args_.bits));
    ...
    bool handle_repl_command(const std::string& cmd, const std::string& arg) {
        bool handled = AppShell<GenericRefComparerArgs, GenericRefComparerState>::handle_repl_command(cmd, arg);
        ...
        if (cmd == "iter") {
            if (arg.empty()) {
                ...
                try {
                    int v = std::stoi(arg);
                    if (v < 4 || v > 64) {
                        std::cout << "  Ошибка: количество итераций должно быть в диапазоне [4, 64]\n";
                    } else {
                        args_.iterations = v;
                        std::cout << "  Кол-во итераций: " << args_.iterations << "\n";
                    }
    ...
    int run_repl() {
        ...
        while (true) {
            ...
            line = AppShell<GenericRefComparerArgs, GenericRefComparerState>::trim(line);
            ...
            if (!line.empty() && line[0] == ':') {
                std::string arg;
                std::string cmd = AppShell<GenericRefComparerArgs, GenericRefComparerState>::parse_cmd(line, arg);
